...
defineConfig({
  test: {
    typecheck: {
      enabled: true,
      only: false,              // Only type check (no runtime tests)
      checker: 'tsc',           // 'tsc' or 'vue-tsc'
      include: ['**/*.test-d.ts'],
...
// toMatchTypeOf - subset matching (B extends A)
expectTypeOf<B>().toMatchTypeOf<A>()

// toEqualTypeOf - exact match
expectTypeOf<A>().not.toEqualTypeOf<B>()
expectTypeOf<A>().toEqualTypeOf<{ x: number }>()
```
