...
Continue test after failure (all failures reported at end):
...
Retry until passes (useful for async UI/state changes):
...
- `expect.soft` continues after failure; all failures reported at end
- `expect.poll` retries with configurable interval and timeout
