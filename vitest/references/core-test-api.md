...
// Or with options object (v4: options are second argument)
...
// Dynamic skip via context (v3.1+: conditional skip)
...
```ts
test('with annotation', ({ annotate }) => {
  annotate('note', 'This test checks the login flow')
  // Annotations are visible to reporters
})
```
...
```ts
test('with signal', ({ signal }) => {
  // signal is an AbortSignal, aborted on timeout, cancellation, or bail
  fetch('/api', { signal })
})
```
...
- v4: Test options are the second argument: `test('name', { retry: 2 }, () => {})`
- Done callback is NOT supported - use async/await or Promises
