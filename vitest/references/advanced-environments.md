...
- `jsdom` - Browser-like with DOM APIs (requires `jsdom` package)
- `happy-dom` - Faster alternative to jsdom (requires `happy-dom` package)
- `edge-runtime` - Vercel Edge Runtime (requires `@edge-runtime/vm` package)
...
Create custom environment package (`vitest-environment-${name}`) or specify file path:
...
builtinEnvironments // { jsdom, 'happy-dom', node, 'edge-runtime' }

// Populate global scope
populateGlobal(global, original, options)
```
...
- Browser Mode is for real browser testing, not environment simulation
