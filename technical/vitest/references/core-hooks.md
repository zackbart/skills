...
Default order (parallel since v4):
...
defineConfig({
  test: {
    sequence: {
      hooks: 'parallel', // 'stack', 'list', 'parallel' (default)
    },
  },
})
```

- `parallel` - Run hooks concurrently (default)
- `stack` - Run in order, teardown in reverse
- `list` - Run in order, teardown in order (Jest behavior)
