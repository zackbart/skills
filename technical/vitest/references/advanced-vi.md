...
```ts
// Wait for callback to succeed (retries until pass)
...
vi.restoreAllMocks() // Restore originals (spies only in v4)
```
...
- v4: `vi.restoreAllMocks` only restores manually-created spies
