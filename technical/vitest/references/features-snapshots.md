...
Vitest updates the test file on first run:
...
## Screenshot Snapshots (Browser Mode)
...
Match partial structure with dynamic values:
...
defineConfig({
  test: {
    snapshotFormat: {
      printBasicPrototype: false,   // Default in Vitest (true in Jest)
      escapeString: false,
...
| Feature | Jest | Vitest |
|---------|------|--------|
| Header | `// Jest Snapshot v1` | `// Vitest Snapshot v1` |
| printBasicPrototype | `true` (default) | `false` (default) |
| Message separator | Colon `:` | Chevron `>` |
| Error snapshot | Stringified message | Full error representation |
